                        ...
                        <tbody>
                            @forelse($applications as $app)
                                <tr class="{{ $app->trashed() ? 'table-secondary' : '' }}">
                                    ...
                                    <td>
                                        @if($app->trashed())
                                            <form method="POST" action="{{ route('ex-supervisor.admin.restore', $app->id) }}"
                                                class="d-inline">
                                                @csrf
                                        ...
                                        @else
                                            <a href="{{ route('ex-supervisor.admin.edit', $app) }}" class="btn btn-sm btn-warning">
                                                <i class="fas fa-edit"></i> Edit
                                            </a>
                                            <form method="POST" action="{{ route('ex-supervisor.admin.destroy', $app) }}"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <div class="modal fade" id="statusModal{{ $app->id }}" tabindex="-1">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form method="POST"
                                                action="{{ route('ex-supervisor.admin.change-status', $app) }}">
                                                @csrf
