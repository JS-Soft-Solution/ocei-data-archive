    ...
    <div class="modal fade" id="bulkRejectModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST" action="{{ route('ex-contractor.office-assistant.bulk-reject') }}">
                    @csrf
    ...
    @push('scripts')
        <script>
            ...
            function bulkApprove() {
                ...
                form.method = 'POST';
                form.action = '{{ route("ex-contractor.office-assistant.bulk-approve") }}';
                form.innerHTML = `
                                                        @csrf
            ...
            function singleApprove(appId) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        const form = document.createElement('form');
                        form.method = 'POST';
                        form.action = `/ex-contractor/office-assistant/${appId}/approve`;
                        form.innerHTML = '@csrf';
                        document.body.appendChild(form);
                        form.submit();
                    }
            ...
            function singleReject(appId) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        const form = document.createElement('form');
                        form.method = 'POST';
                        form.action = `/ex-contractor/office-assistant/${appId}/reject`;
                        form.innerHTML = `
                                    @csrf
