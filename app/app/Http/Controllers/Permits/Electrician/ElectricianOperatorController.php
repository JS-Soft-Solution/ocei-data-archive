...
class ElectricianOperatorController extends Controller
{
    ...
    {
        $this->authorize('viewAny', ExElectricianRenewApplication::class);
    ...
    {
        $this->authorize('create', ExElectricianRenewApplication::class);

        return view('permits.electrician.operator.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $application = ExElectricianRenewApplication::create([
                ...$validated,
                'status' => 'draft',
                'entry_by' => Auth::id(),
                'entry_at' => now(),
            ...
            return redirect()
                ->route('ex-electrician.operator.edit', $application)
    ...
    public function show(ExElectricianRenewApplication $application)
    {
        ...
        return view('permits.electrician.operator.show', compact('application'));
    }
    ...
    public function edit(ExElectricianRenewApplication $application)
    {
        ...
        return view('permits.electrician.operator.edit', compact('application'));
    }
     ...
     */
    public function saveTab(Request $request, ExElectricianRenewApplication $application)
    {
        $this->authorize('update', $application);
        ...
        try {
            ...
            return redirect()
                ->route('ex-electrician.operator.edit', ['application' => $application, 'tab' => $nextTab])
     ...
     */
    public function submit(ExElectricianRenewApplication $application)
    {
        ...
        try {
            ...
            return redirect()
                ->route('ex-electrician.operator.index')
    ...
    public function claim(Request $request)
    {
        ...
        $application = ExElectricianRenewApplication::where('old_certificate_number', $certificateNumber)->first();

        // Case 1: Certificate not found - Create new entry
        if (!$application) {
            return redirect()
                ->route('ex-electrician.operator.create', ['old_certificate_number' => $certificateNumber])
        ...
        if ($application->status === 'secretary_approved_final') {
            if ($isSuperAdmin) {
                return redirect()
                    ->route('ex-electrician.operator.show', $application)
     ...
     */
    protected function validateApplication(Request $request, ?ExElectricianRenewApplication $application = null): array
    {
        $uniqueRule = $application
            ? 'unique:ex_electrician_renew_applications,old_certificate_number,' . $application->id
            : 'unique:ex_electrician_renew_applications,old_certificate_number';
