...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create('ex_electrician_renew_applications', function (Blueprint $table) {
            ...
            // Foreign keys with short custom names (MySQL 64-char limit)
            $table->foreign('entry_by', 'ex_elec_entry_fk')->references('id')->on('users')->onDelete('set null');
            $table->foreign('last_updated_by', 'ex_elec_upd_fk')->references('id')->on('users')->onDelete('set null');
            $table->foreign('rejected_by', 'ex_elec_rej_fk')->references('id')->on('users')->onDelete('set null');
            $table->foreign('verified_by_office_assistant', 'ex_elec_oa_fk')->references('id')->on('users')->onDelete('set null');
            $table->foreign('approved_by_secretary', 'ex_elec_sec_fk')->references('id')->on('users')->onDelete('set null');
            $table->foreign('deleted_by', 'ex_elec_del_fk')->references('id')->on('users')->onDelete('set null');
        });
    }
